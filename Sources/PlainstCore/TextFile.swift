...
public struct TextFile: Sendable {
  ...
  public init(data: Data, encoding requestedEncoding: TextEncoding? = nil) throws {
    ...
    let normalized = Self.normalize(decoded.text)
    text = normalized
    encoding = decoded.encoding
    lineEnding = endings.preferred
    hasMixedLineEndings = endings.mixed
    original = requestedEncoding == nil
      ? Original(
        data: data, text: normalized, encoding: decoded.encoding, lineEnding: endings.preferred)
      : nil
  }
  ...
  public func data(
    ...
    if outputEncoding == nil, outputLineEnding == nil, let original,
      text == original.text, encoding == original.encoding, lineEnding == original.lineEnding
    {
  ...
  private static func decode(_ data: Data, requestedEncoding: TextEncoding?) throws -> (
    ...
    throw CocoaError(
      .fileReadInapplicableStringEncoding,
      userInfo: [NSLocalizedDescriptionKey: "Plainst could not determine the file's text encoding."]
    )
  }
